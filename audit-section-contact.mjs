// Contact section + footer audit. Static, no states.
...
const OUT = join(__dirname, 'audit-section-contact-shots');
...
for (const { name, viewport } of VIEWPORTS) {
    ...
    for (const sel of ['#contact', 'footer.footer, .footer']) {
        const loc = page.locator(sel).first();
        if (!(await loc.count())) continue;
        await loc.scrollIntoViewIfNeeded();
        await page.waitForTimeout(300);
        const box = await loc.boundingBox();
        if (box) await page.screenshot({ path: join(OUT, `${name}-${sel.replace(/[^a-z0-9]/gi,'_')}.png`), clip: box });
