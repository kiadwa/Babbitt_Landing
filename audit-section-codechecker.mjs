// CodeChecker section audit — static cards, no state cycling.
...
const OUT = join(__dirname, 'audit-section-codechecker-shots');
...
for (const { name, viewport } of VIEWPORTS) {
    ...
    await page.locator('#codechecker').scrollIntoViewIfNeeded();
    await page.waitForTimeout(400);
    const box = await page.locator('#codechecker').boundingBox();
    if (box) await page.screenshot({ path: join(OUT, `${name}.png`), clip: box });
    const { probes, error } = await runProbe(page, '#codechecker');
    if (error) { console.log(error); continue; }
    const issues = scoreProbes(probes);
    printReport(name, viewport, '#codechecker', probes, issues);
