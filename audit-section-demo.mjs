// Demo selector audit — six role chips, no carousel/state-cycle.
// One snapshot per viewport suffices.
...
const OUT = join(__dirname, 'audit-section-demo-shots');
...
for (const { name, viewport } of VIEWPORTS) {
    ...
    const { probes, error } = await runProbe(page, '#demo');
    if (error) { console.log(error); continue; }
    const issues = scoreProbes(probes);
    printReport(name, viewport, '#demo', probes, issues);
